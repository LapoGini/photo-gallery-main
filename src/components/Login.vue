    ...
    <ion-page>
        ...
        <ion-content>
          <form >
              ...
              <ion-item class="authentication-error" v-if="authenticationError" color="danger">
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    async mounted() {
      ...
      if (savedApiToken) {
          axios.defaults.headers.common['Authorization'] = 'Bearer ' + savedApiToken;
      }
      if (this.hasApiToken) {
    ...
    methods: {
      ...
      async eliminaDatabase(databaseName: string) {
        try {
          ...
          console.error(`Errore nell'eliminazione del database ${databaseName}:`, errore);
        }
      },
      async login() {
        this.errorMessage = '';
        try {
          ...
          if (error && error.message === 'Network Error') {
            this.errorMessage = 'É richiesta una connessione ad internet per effettuare il login!';
          } else {
            this.errorMessage = 'Errore di autenticazione: email o password errate!';
          }
        } finally {
